mu_f <- faultCount ~ 
  ...
  bols(wind_direction, intercept = FALSE, df = 1) + # Unordered
  bols(lightning, intercept = FALSE, df = 1) + # Ordinal
  
  # Continuous Variables
  bols(wind_gust, intercept = FALSE) +
  bbs(wind_gust, center = TRUE, df = 1) +
  ...
  # Gust X Direction #
  bols(wind_gust, by = wind_direction,
       intercept = FALSE, df = 1) +
  bbs(wind_gust, by = wind_direction,
      center = TRUE, df = 1) +
  # Gust X Region
  bols(wind_gust, by = regionCode, intercept = FALSE, df = 1) +
  bbs(wind_gust, by = regionCode, center = TRUE, df = 1) +
  
  # Mean X Direction #
  bols(wind_mean, by = wind_direction,
       intercept = FALSE, df = 1) +
  bbs(wind_mean, by = wind_direction,
      center = TRUE, df = 1) +
  # Mean X Region
  ...
  bols(wind_gust, by = dir_X_regionCode,
       intercept = FALSE, df = 1) +
  bbs(wind_gust, by = dir_X_regionCode,
      center = TRUE, df = 1) +
