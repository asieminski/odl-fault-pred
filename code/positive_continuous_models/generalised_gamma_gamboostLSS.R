...
mod <- gamboostLSS(formula = full_f[1:2], 
                  data = dt_fault_trans, method = "noncyclic", 
                     families = as.families(GA(), stabilization = "MAD"))
...
while(converging){
  ...
  if(error){
    ...
    if(old_min_risk == new_min_risk){
      ...
      converging <- FALSE
      saveRDS(object, "res/positive_continuous_models/GAmod.rds")
    }
  }  
}
# OLD_MIN_RISK = NEW_MIN_RISK
# LAST LogLik: -1244.371
# 1'552
